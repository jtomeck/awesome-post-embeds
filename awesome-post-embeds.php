...
function tag_id_from_slug( $api_url, $slug ) {
  $tag = json_decode (
    file_get_contents( $api_url . "/tags?slug=$slug" )
  );
  return $tag[0]->id;
}
...
function category_id_from_slug( $api_url, $slug ) {
  $category = json_decode (
    file_get_contents( $api_url . "/categories?slug=$slug" )
  );
...
function ape_posts_func( $atts ) {
  ...
  if( $atts['url'] ) { // Require a URL in shortcode
    ...
    * POST STRUCTURE
    *
    * This is the structure of the returned posts with important variables injected in
    */
